...
export async function POST(request: Request) {
  try {
    ...
    const configResult = await query<{baseUrl:string;model:string;encryptedKey:string}>(`SELECT base_url AS "baseUrl",model,encrypted_key AS "encryptedKey" FROM ai_configs WHERE user_id=$1`, [user.id]);
