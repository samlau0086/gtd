...
export async function getState(userId:string) {
  const [projects,tasks,tags,version] = await Promise.all([
    query<any>(`SELECT id,name,color,background_color AS "backgroundColor",text_color AS "textColor",border_color AS "borderColor",revision,updated_at AS "updatedAt" FROM projects WHERE user_id=$1 ORDER BY created_at`,[userId]),
...
export async function createProject(userId:string,input:{name:string;color?:string;backgroundColor?:string;textColor?:string;borderColor?:string;id?:string}) { return withTransaction(async(client)=>{const id=input.id||randomUUID();if(!input.name?.trim())throw new GtdError("项目名称不能为空");const backgroundColor=hexColor(input.backgroundColor,"#173F3B"),textColor=hexColor(input.textColor,"#D9FFF9"),borderColor=hexColor(input.borderColor,hexColor(input.color,"#69D2C8"));await client.query("INSERT INTO projects(id,user_id,name,color,background_color,text_color,border_color,revision,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,1,NOW(),NOW())",[id,userId,input.name.trim().slice(0,80),borderColor,backgroundColor,textColor,borderColor]);const dataVersion=await bumpDataVersion(client,userId);const record=(await client.query<any>('SELECT id,name,color,background_color AS "backgroundColor",text_color AS "textColor",border_color AS "borderColor",revision,updated_at AS "updatedAt" FROM projects WHERE id=$1',[id])).rows[0];return{record:{...record,updatedAt:new Date(record.updatedAt).toISOString()},dataVersion};}); }
export async function updateProject(userId:string,id:string,expectedRevision:number,input:{name?:string;color?:string;backgroundColor?:string;textColor?:string;borderColor?:string}) { return withTransaction(async(client)=>{const backgroundColor=input.backgroundColor?hexColor(input.backgroundColor,"#173F3B"):null,textColor=input.textColor?hexColor(input.textColor,"#D9FFF9"):null,borderColor=input.borderColor||input.color?hexColor(input.borderColor||input.color,"#69D2C8"):null;const result=await client.query<any>(`UPDATE projects SET name=COALESCE($1,name),color=COALESCE($4,color),background_color=COALESCE($2,background_color),text_color=COALESCE($3,text_color),border_color=COALESCE($4,border_color),revision=revision+1,updated_at=NOW() WHERE id=$5 AND user_id=$6 AND revision=$7 RETURNING id,name,color,background_color AS "backgroundColor",text_color AS "textColor",border_color AS "borderColor",revision,updated_at AS "updatedAt"`,[input.name?.trim().slice(0,80)||null,backgroundColor,textColor,borderColor,id,userId,expectedRevision]);if(!result.rowCount){const current=(await client.query<any>('SELECT id,name,color,background_color AS "backgroundColor",text_color AS "textColor",border_color AS "borderColor",revision,updated_at AS "updatedAt" FROM projects WHERE id=$1 AND user_id=$2',[id,userId])).rows[0];throw new GtdError(current?"项目已在其他客户端更新":"项目不存在",current?409:404,{current});}const dataVersion=await bumpDataVersion(client,userId);return{record:{...result.rows[0],updatedAt:new Date(result.rows[0].updatedAt).toISOString()},dataVersion};}); }
...
export async function listProjects(userId:string,options:{cursor?:string;limit?:number}={}) {const limit=Math.max(1,Math.min(100,options.limit||50)),cursor=decodeCursor(options.cursor),values:unknown[]=[userId];let clause="";if(cursor){values.push(cursor);clause=`AND id>$${values.length}`;}values.push(limit+1);const rows=(await query<any>(`SELECT id,name,color,background_color AS "backgroundColor",text_color AS "textColor",border_color AS "borderColor",revision,updated_at AS "updatedAt" FROM projects WHERE user_id=$1 ${clause} ORDER BY id LIMIT $${values.length}`,values)).rows;const items=rows.slice(0,limit).map((x)=>({...x,revision:Number(x.revision),updatedAt:new Date(x.updatedAt).toISOString()}));return{items,nextCursor:rows.length>limit?Buffer.from(items.at(-1)!.id).toString("base64url"):null,dataVersion:await getDataVersion(userId)};}
